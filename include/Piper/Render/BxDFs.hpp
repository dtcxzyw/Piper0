...
public:
    ...
    BSDFSample sample(SampleProvider& sampler, const Direction& wo, const TransportMode transportMode,
        ...
        return {
            wi,
            importanceSampled<PdfType::BSDF>(evaluate(wo, wi, transportMode)),
    ...
    BSDFSample sample(SampleProvider& sampler, const Direction& wo, const TransportMode transportMode,
        ...
        if(eta == 1 || mfDistrib.EffectivelySmooth()) {
            ...
            if(sampler.sample() < pr / (pr + pt)) {
                ...
                return {
                    wi,
                    importanceSampled<PdfType::BSDF>(ft * Rational<Spectrum>::identity()),
                    InversePdfValue::fromRaw((pr + pt) / pr),
                ...
                return {
                    wi,
                    importanceSampled<PdfType::BSDF>(ft * Rational<Spectrum>::identity()),
                    InversePdfValue::fromRaw((pr + pt) / pt),
                    BxDFPart::SpecularTransmission
                };
            }
        } else {
